...
Public Sub Crea_Rs()
  ...
  '  rs.AddNew
  '  rs!ESTRUCTURA = "ESTRUCTURA 22: Datos de Inasistencias "
  '  rs!NRO = 22
...
'    Rs!NRO = 22
    ...
    rs.AddNew
    rs!ESTRUCTURA = "ESTRUCTURA 25: Datos de Inasistencias "
    rs!NRO = 25
End Sub
...
If fAbrRst(Rq, Sql) Then
    ...
    Do While Not Rq.EOF
        ...
        'add jcms solciitado por Mlopez 071016
        Reg = Reg & vSeparador & IIf(chkEsPlame.Value = 1, Trim(Left(Rq!serie_comprob & "", 4)), Trim(Mid(Rq!serie_comprob & "", 1, 4)))
        ...
        Dim sRegimen As String
        sRegimen = ""
        
        If Rq!ImpTasaRetenido <> 0 Then
'            If Trim(Rq!codRegAfp) = "01" Then 'impuesto
'                Reg = Reg & vSeparador & "1"
'            ElseIf Trim(Rq!codRegAfp) <> "01" And Trim(Rq!codRegAfp) <> "" Then  'impuesto
'                Reg = Reg & vSeparador & "2"
'            ElseIf Trim(Rq!codRegAfp & "") = "" Then 'impuesto
'                Reg = Reg & vSeparador & "3"
'            End If
            'add jcms 120416 naty pidio que se quite el valor
            Reg = Reg & vSeparador & ""
            
            If Trim(Rq!codRegAfp) = "01" Then '
                Reg = Reg & vSeparador & Format((Rq!total * Rq!ImpTasaRetenido) / 100, "#######.00")
            Else
                Reg = Reg & vSeparador
            End If
        Else
...
Public Sub Exp_Estructura18_solo_vida()
'/*ULIMO CAMBIO ADD JCMS 05/08/09*/
'/*jcbr solicito acumular las boletas de gratificacion en el concepto de sunat 0406 excepto el codigo 0312*/
...
'/*add seguro vida*/
   ...
   Sql = "[usp_pla_exporta_planilla_electronica_plame_seguro_vida] '" & wcia & "'," & xPeriodoAño & "," & xPeriodoMes & IIf(Trim(xUnTrabajador) <> "", ",'" & xUnTrabajador & "'", ",'*'") & ",'" & xFechaLimiteFinPeriodo & "'"
   If fAbrRst(Rv, Sql) Then
        Do While Not Rv.EOF
                rsRem.AddNew
                rsRem!tipo_doc = Trim(Left(Rv!tipo_doc & "", 2))
                rsRem!nro_doc = Trim(Rv!nro_doc & "")
                rsRem!cod_sunat = Trim(Rv!cod_sunat & "")
                rsRem!monto_devengado = Rv!importe
                rsRem!monto_pagado = Rv!importe
            Rv.MoveNext
        Loop
   Else
        MsgBox "Falta importes del seguro VIDA para el periodo,verifique", vbExclamation, Me.Caption
   End If
   Rv.Close
   Set Rv = Nothing
    ...
    With rsRem
        If .RecordCount > 0 Then
            ...
            Do While Not .EOF
                    ...
                    Reg = Reg & vSeparador & Trim(Left(!nro_doc & "", 15))
                    Reg = Reg & vSeparador & Right(!cod_sunat, 4)
                    Reg = Reg & vSeparador & IIf(!monto_devengado <> 0 Or chkEsPlame.Value = 0, Format(!monto_devengado, "#######.00"), "0.00")
                    Reg = Reg & vSeparador & IIf(!monto_pagado <> 0 Or chkEsPlame.Value = 0, Format(!monto_pagado, "#######.00"), "0.00")
                    Reg = Reg & vSeparador
                    Print #1, Reg
                    i = i + 1
                    Barra.Value = i
            ...
            If Rq.Fields(intloop).Value <> 0 Or Rq.Fields(intloop).Name = "d13" Then ' Or Rq.Fields(intloop).Name = "a03") Then  'Rq.Fields(intloop).Name = "d04" Or Rq.Fields(intloop).Name = "d11"
                 ...
                 With rsRem
                    ...
                    Else
                        xTipo = "03" 'deducciones
                    End If
                    If Rq.Fields(intloop).Name = "i30x" Then
                       xCodSunat = "0313"
                    Else
                       If Len(Rq.Fields(intloop).Name) = 3 Then
                           xCodSunat = Buscar_CodSunat(RsConcep, xTipo & Trim(Right(Rq.Fields(intloop).Name, 2)))
...
'                            Print #1, Reg ' & "***"
                    ...
                    'ADD JCMS 140113
                    If Right(!cod_sunat, 4) = "0312" Or Right(!cod_sunat, 4) = "0809" Or Right(!cod_sunat, 4) = "0810" Or Right(!cod_sunat, 4) = "0605" Or Right(!cod_sunat, 4) = "0706" Or Right(!cod_sunat, 4) = "0703" Then     '/*bonificacion extraordinaria,SCTR - S,SCTR - P  5TA CAT GRATI,CTACTE,JUDICIAL*/
...
Public Sub Exp_Estructura18_roda()
'/*ULIMO CAMBIO ADD JCMS 05/08/09*/
'/*jcbr solicito acumular las boletas de gratificacion en el concepto de sunat 0406 excepto el codigo 0312*/
...
Public Sub Exp_Estructura25()
'//*** ESTRUCTURA 25: "Dias Falta"
...
Dim i As Integer
i = 0
xRutaFile = App.Path & "\reports\0601" & Mid(Format(dFechaSOL, "YYYYMMDD"), 1, 6) & xRucCia & ".SNL"
Dim Rq As ADODB.Recordset
Sql = "usp_pla_exporta_planilla_electronica_Faltas '" & wcia & "'," & xPeriodoAño & "," & xPeriodoMes & ",'O'"
Open xRutaFile For Output As #1
Screen.MousePointer = 11
If fAbrRst(Rq, Sql) Then
    Barra.Max = Rq.RecordCount
    ...
    Do While Not Rq.EOF
        ...
        Reg = Trim(Left(Rq!tipo & "", 2))
        Reg = Reg & vSeparador & Trim(Left(Rq!DNI & "", 12))
        Reg = Reg & vSeparador & Trim(Left(Rq!Codsunat & "", 2))
        Reg = Reg & vSeparador & Trim(Left(Rq!Dias & "", 5))
