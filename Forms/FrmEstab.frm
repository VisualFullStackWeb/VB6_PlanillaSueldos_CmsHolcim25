...
ElseIf Len(Trim(TxtDenominacion.Text)) > 40 Then
    MsgBox "Ingrese Denominación máx 40 caracteres.", vbExclamation, Me.Caption
    TxtDenominacion.SetFocus
    Exit Sub
...
Select Case vAccion
    Case 1 'nuevo
        If Frmcia.RsEstablecimientos.RecordCount > 0 Then
            Frmcia.RsEstablecimientos.MoveFirst
            Frmcia.RsEstablecimientos.FIND "CODEST='" & Trim(TxtCodigo.Text) & "'"
            If Not Frmcia.RsEstablecimientos.EOF Then
        ...
        Frmcia.RsEstablecimientos.AddNew
End Select
Frmcia.RsEstablecimientos!codest = Trim(Me.TxtCodigo.Text)
Frmcia.RsEstablecimientos!nomest = Trim(Me.TxtDenominacion.Text)
Frmcia.RsEstablecimientos!tipest = fc_CodigoComboBox(CmbTipEst, 2)
Frmcia.RsEstablecimientos!nomtipest = CmbTipEst.Text
Frmcia.RsEstablecimientos!centro_riesgo = IIf(OptSi(0).Value = True, True, False)

With RsTasa
   
        If Frmcia.RsEstTasa.RecordCount > 0 Then Frmcia.RsEstTasa.MoveFirst
        Do While Not Frmcia.RsEstTasa.EOF
            If Trim(Frmcia.RsEstTasa!codest) = Trim(Me.TxtCodigo.Text) Then
                Frmcia.RsEstTasa.Delete
            End If
           Frmcia.RsEstTasa.MoveNext
        Loop
     If .RecordCount > 0 Then
        .MoveFirst
        Do While Not .EOF
                Frmcia.RsEstTasa.AddNew
                Frmcia.RsEstTasa!codest = Trim(Me.TxtCodigo.Text)
                Frmcia.RsEstTasa!porc = Format(!porc, "##0.00")
            .MoveNext
        Loop
    End If
End With
