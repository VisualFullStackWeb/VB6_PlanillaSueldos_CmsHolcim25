...
Begin VB.Form Frmafpaunal 
   ...
   Caption         =   "» Liquidación Anual de Aportes de AFP «"
   ClientHeight    =   1170
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   5160
...
' End If
 ...
 Sql$ = "EXEC SP_REPORTE_AFP '" & wcia & "'," & CInt(Val(Trim(Txtano.Text))) & ",'" & Trim(vAfp) & "'"
If Not Ejecuta(Sql$) Then MsgBox "Error Critico", vbInformation, "Liquidacion Anual de Afp": Exit Sub

Sql$ = "SELECT ID AS PLACOD,CODIGO AS NUMAFP," & _
        "SUM(ENERO) AS M01," & _
...
Do While Not rs.EOF
   ...
   mcad = fCadNum(mItem, "###") & " " & lentexto(12, Left(NUMAFP, 12)) & " " & lentexto(17, Left(rs2!ap_pat, 17)) & " " & lentexto(17, Left(rs2!ap_mat, 17)) & " " & lentexto(17, Left(Trim(rs2!nom_1) & " " & Trim(rs2!nom_2), 17)) & " " & Trim(rs!PlaCod) & " " & Format(rs2!fcese, "dd/mm/yyyy")
   For I = 2 To 13
   ...
   mcad = mcad & Space(1) & fCadNum(mtot, "###,###.00")
   Print #1, mcad
   If rs2.State = 1 Then rs2.Close
   rs.MoveNext
   mItem = mItem + 1
...
Print #1, Space(87) & String(143, "-")

mcad = Space(88) & fCadNum(mtot1, "###,###.00") & Space(1) & fCadNum(mtot2, "###,###.00") & Space(1) & fCadNum(mtot3, "###,###.00") & Space(1) & fCadNum(mtot4, "###,###.00") & Space(1) & fCadNum(mtot5, "###,###.00") & Space(1)
...
Print #1,
Print #1, " No    CUSPP     Apellido Paterno  Apellido Materno  Nombre            Código    F.Cese     Ene        feb        Mar        Abr        May        Jun        Jul        Ago        Sep        Oct         Nov        Dic        Total"
Print #1, String(238, "-")
mlinea = 11
End Sub
