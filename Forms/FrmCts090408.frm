        ...
        If CInt(rs!sn_carga) <> 0 Then
            ...
            If Mid(rs!CAMPUNION, 2, 2) = "29" Then
                'CADENA = CADENA & " CASE WHEN (SELECT COUNT(ISNULL(I29,0)) FROM PLAHISTORICO PH WHERE PH.CIA ='" & wcia & "' and ph.status!='*'  and  ph.FECHAPROCESO>='" & FecIni & "' AND ph.FECHAPROCESO<='" & FecFin & "' and ph.placod=a.placod AND PH.PROCESO='01') >=3 THEN " & _
                 '        "ISNULL((SELECT SUM(COALESCE(I29,0)) FROM plahistorico ph WHERE ph.cia='" & wcia & "' and ph.status!='*'  and  ph.FECHAPROCESO>='" & FecIni & "' AND ph.FECHAPROCESO<='" & FecFin & "' and ph.placod=a.placod AND PH.PROCESO='01')/6,0) ELSE 0 END as P29"
                         
            CADENA = CADENA & "CASE WHEN ( SELECT COUNT(ISNULL(PH.I29,0)) AS CONTEO " & _
                                        " FROM PLAHISTORICO PH" & _
                                        " WHERE PH.CIA ='" & wcia & "'" & _
                                        " AND ph.status!='*'" & _
                                        " AND  ph.FECHAPROCESO>='" & FecIni & "'" & _
