...
Begin VB.Form FrmEmpTraePer 
   ...
   End
End
Attribute VB_Name = "FrmEmpTraePer"
...
Select Case vAccion
    Case 1 'nuevo
        If Frmcia.RsEmpTraePer.RecordCount > 0 Then
            Frmcia.RsEmpTraePer.MoveFirst
            Frmcia.RsEmpTraePer.FIND "ruc='" & Trim(TxtRuc.Text) & "'"
            If Not Frmcia.RsEmpTraePer.EOF Then
                MsgBox "El Número de Ruc ya fue ingresado", vbExclamation, Me.Caption
                TxtRuc.SetFocus
                Exit Sub
            End If
        End If
        Frmcia.RsEmpTraePer.AddNew
End Select
Frmcia.RsEmpTraePer!RUC = Trim(TxtRuc.Text)
Frmcia.RsEmpTraePer!razsoc = Trim(TxtRazsoc.Text)

'actividad y/o servicio
With RsServ
    
        If Frmcia.RsEmpTraePer_Actividad.RecordCount > 0 Then Frmcia.RsEmpTraePer_Actividad.MoveFirst
        Do While Not Frmcia.RsEmpTraePer_Actividad.EOF
            If Trim(Frmcia.RsEmpTraePer_Actividad!RUC) = Trim(Me.TxtRuc.Text) Then
                Frmcia.RsEmpTraePer_Actividad.Delete
            End If
           Frmcia.RsEmpTraePer_Actividad.MoveNext
        Loop
    If .RecordCount > 0 Then
        .MoveFirst
        Do While Not .EOF
                Frmcia.RsEmpTraePer_Actividad.AddNew
                Frmcia.RsEmpTraePer_Actividad!RUC = Trim(Me.TxtRuc.Text)
                Frmcia.RsEmpTraePer_Actividad!cod_serv = Trim(!cod_serv)
            .MoveNext
        Loop
    End If
End With
